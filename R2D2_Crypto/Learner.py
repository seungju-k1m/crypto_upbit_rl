from R2D2.Player import FIXED_TRAJECTORY
from configuration import *
from baseline.baseAgent import baseAgent
from baseline.utils import getOptim, writeTrainInfo

from R2D2_Crypto.ReplayMemory import Replay
...
class Learner:
    ...
    def train(self, transition, t=0) -> dict:
        ...
        hidden_state, state, ratio, action, reward, done, weight, idx = transition
        # state -> SEQ * BATCH
        ...
        state = state / 255.
        ratio = torch.tensor(ratio).to(self.device).float()
        # BATCH, FIXED
        ratio = ratio.permute(1, 0).contiguous()
        ratio_truncated = ratio[MEM:].contiguous()
        ratio_truncated = ratio_truncated.view(-1, 1)
        ratio_burn_in = ratio[:MEM].contiguous()
        ratio_burn_in = ratio_burn_in.view(-1, 1)
        ...
        _, _, channel, hei, wid = truncated_state.shape
        truncated_state = truncated_state.view(-1, channel, hei, wid)
    ...
    def run(self):
        ...
        Mean_Weight:{:.3f}  // MAX_WEIGHT:{:.3f}  // TIME:{:.3f} // TRAIN_TIME:{:.3f} // SAMPLE_TIME:{:.3f} // UPDATE_TIME:{:.3f}""".format(
                ...
                mean_value, norm = 0, 0
                mean_weight = 0
                torch.save(self.state_dict, './weight/r2d2_cpt/weight.pth')
