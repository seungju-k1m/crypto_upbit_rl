...
from configuration import INDICT_LOGGER
...
def load_ema(alpha: Union[List[float], float, None]) -> \
    ...
    if alpha is None:
        ...
        INDICT_LOGGER.info(f"EMA smoothing ENABLED: {alpha}")
...
def apply_ema_all_data(
    ...
    if isinstance(ema, ExponentialMovingAverage):
        INDICT_LOGGER.info("Applying EMA to data...")
        ...
        INDICT_LOGGER.info("Applying list of EMAs to data...")
...
def reset_ema(ema: Union[List[ExponentialMovingAverage], ExponentialMovingAverage, None]) -> \
    ...
    if ema is None:
        ...
        ema.reset()
        INDICT_LOGGER.info("Reset EMA data.")
    elif isinstance(ema, list):
        for e in ema:
            e.reset()
        INDICT_LOGGER.info("Reset EMA data.")
