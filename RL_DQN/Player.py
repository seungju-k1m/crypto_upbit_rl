...
class LocalBuffer:
    ...
    def get_traj(self, done=False):
        ...
        traj += [done]
        traj_ = deepcopy(traj)
        traj_[0] = base64.b16encode(pickle.dumps(traj_[0]))
        traj_[-2] = base64.b16encode(pickle.dumps(traj_[-2]))
...
class Player():
    ...
    def calculate_priority(self, traj):
        with torch.no_grad():
            s, a, r, s_, d = traj
            s = torch.tensor([pickle.loads(base64.b16decode(s))]).float().to(self.device)
            s = s/255.
            s_ = torch.tensor([pickle.loads(base64.b16decode(s_))]).float().to(self.device)
            s_ = s_/255.
