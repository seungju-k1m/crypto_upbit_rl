...
from Simulator.renderer_v2 import Renderer
from Simulator.pipeline_v2 import *
...
def generate_random_start(day):
    data_list = os.listdir('./data/v2')
    data_list.sort()
    total_len = len(data_list)

    start_id = random.randint(1, total_len-4)
...
class Simulator:
    ...
    def reset(self, test=False, port=None):
        ...
        current_price = float(self.renderer.trade_vec[1][-1])
        if port is None:
    ...
    def step(self, action=0):
        """
            Action Space
                ...
                8 -> COIN -> KRW (25%)
        """
        obs, done = self.renderer.step(duration=self.unit)
        _, trade_vec = obs

        current_price = trade_vec[1][-1]
        ...
        return (obs[0], info), reward, done, idle
