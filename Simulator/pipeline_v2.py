...
class DataPipeLine:
    ...
    def __init__(
        ...
        if test:
            ...
            self.data = self.preprocess_raw_data(to, duration, 'test_v2')
        else:
            self.data = self.preprocess_raw_data(to, duration, 'v2')
...
class DataPipeLine_Sim:
    ...
    def process(data:pd.DataFrame):
        ...
        output = [data['time'].to_numpy(), data['opening_price'].to_numpy(), data['high_price'].to_numpy(), 
            data['low_price'].to_numpy(), data['trade_price'].to_numpy(), 
            data['acc_volume'].to_numpy()]
        return output
